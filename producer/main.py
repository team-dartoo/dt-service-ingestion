...
MAX_FAIL = int(os.getenv("MAX_FAIL", "3"))
MIN_FILE_SIZE_BYTES = 200
FAILED_LOG_DIR = os.getenv("FAILED_LOG_DIR")
FAILURE_RECORDER = FailureRecorder(log_dir=FAILED_LOG_DIR)
...
def process_document(api: DartApiClient, store: MinioClient, doc: Disclosure, polling_date: str):
    ...
    if doc.rcept_no in PROCESSED_RCEPT_NOS:
        return 
    if doc.rcept_no in PERM_FAILED:
        return
    ...
    try:
        ...
        if store.upload_document(object_name, normalized_body, content_type):
            ...
            PROCESSED_RCEPT_NOS.add(doc.rcept_no)
            celery_app.send_task('tasks.summarize_report', args=[doc.rcept_no, object_name])
